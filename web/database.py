...
Uses SQLite with Fernet encryption for sensitive data.
"""
...
def decrypt_value(encrypted: str) -> str:
    ...
    # Unencrypted value (legacy or plain text)
    return encrypted
...
def list_credentials(include_secrets: bool = False) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if include_secrets:
            cred['password'] = decrypt_value(row['password_encrypted']) if row['password_encrypted'] else None
            cred['ssh_key'] = decrypt_value(row['ssh_key_encrypted']) if row['ssh_key_encrypted'] else None
...
def list_env_variables(include_values: bool = False) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if include_values:
            var['value'] = decrypt_value(row['value_encrypted'])
        else:
            var['value'] = '********'
...
def load_env_variables_to_environ():
    ...
    for var in list_env_variables(include_values=True):
        if var['key'] not in os.environ:  # Don't override existing
